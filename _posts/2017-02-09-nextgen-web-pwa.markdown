...
## MicroX vs In-Memory Fuzzing
#### 区别：
- micro execution更像是一种虚拟执行，直接定位到某个函数后，无论是需要访问指针还是具体数据都是通过策略模拟构造出来的,
in-memory fuzzing是实际执行，除了需要构造的覆盖的数据以外，其余的都是使用的真是数据，这样测试结果才能更加真实。
- micro execution和普通非单元测试的区别,
全局的遍历测试消耗大，及时是使用符号执行也很难有效划分区域，判断哪些input段可以修改，且能够进行有效的测试。

## 思考
micro对整个执行过程中所有内存访问指令的监控，一方面可以很容易判断执行过程中会访问哪些和输入参数相关的内存区域，但也存在下面的问题问题：
- 参数结构复杂性, 函数调用时，pointer或者data作为参数的情况很少，pointer指向未知结构体的情况很多，且在一个函数中只会访问结构体重的部分字段，这样的话当程序执行到函数外部时很难进行下去，因为开始测试时并没有构造完整的结构体，只测试函数内部的代码本来就不能监控到所有异常，sumatra这个例子就很好的说明了这个问题，所以memory fuzzing这种动态的方式显然更有优势。
- 函数调用关系，或者函数见得嵌套对实际测试中的影响，micro可能把外部调用函数的代码全部当作本函数的代码进行分析，这样的话如果是外部系统函数调用一定会出现问题，如果是非内核监控模式调试的话。
- micro execut这个文章中并没有提到过一致性的问题，即使提到这个问题，我们该怎样检测或者诊断？崩溃时由于一致性问题导致的
另外micro中对函数参数的确定相对比较简单，如果是usercall的形式，单靠这种方式是否能够完全识别出来呢？好像也可以，因为监控了所有内存访问操作。
- 要对全指令插装分析，目前我们也是这样做的，但是指令级别的插装对效率的影响是极大的。
